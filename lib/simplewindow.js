/**
 *
 *  a Simple Window  library based on the prototype/scriptaculous libraries
 *
 *  created on 17 March, 2009
 *  last update on 11 Nov, 2010
 *
...
var $sw = {
	...
	defaultOptions: {
	  ...
	  //    the parameters are same as Ajax.Request in prototype.js.
		...
		// callback function will be called before open the simple window if have.
		beforeOpen: null,

		// callback function will be called after open the simple window if have.
		afterOpen: null,

		// callback function will be called before close or remove the simple window if have.
		beforeClose: null,

		// callback function will be called after close or remove the simple window if have.
		afterClose: null,
	 ...
	 *
	 */
	changeContent: function(sw, content, options) {
	  this._sw = sw = $(sw);
		if (!sw) {
			return;
		}
		...
		// before change
		if (Object.isFunction(options.beforeChange)) {
			options.beforeChange(sw);
		}
		
		// set content
		sw.update(this._cache.headerContent[this._sw.id] + 
			content + this._cache.footerContent[this._sw.id]).setStyle({width:'',height:''});
    ...
    if (options.changePosition) {
      sw.setStyle({
		...
		// after change
		if (Object.isFunction(options.afterChange)) {
			options.afterChange(sw);
		}
	},
	...
	_renderOverlay: function() {
		...
		this._overlay.setStyle({
			...
			width: self._docDim.width + 'px',
			height: self._docDim.height + 'px'
		});
	...
	_getPositionAndSize: function() {
		...
		if (null === _options.size) {
			...
			var _swBP = this._getBlankSpaces(this._sw),
			    _swDim = this._sw.setStyle(this._setBlankSpaces(_swBP, '0')).getDimensions();
		...
		// position offset
		if (null != _options.positionOffset) {
			if (_options.positionOffset.include('x')) {
				var _offset = _options.positionOffset.split('x');
				posLeft += parseInt(_offset[0]);
				posTop += parseInt(_offset[1]);
			} else if (typeof _options.positionOffset == 'object') {
				posLeft += _options.positionOffset.left;
				posTop += _options.positionOffset.left;
			}
		}

		// fix position, so it doesn't affacted by the parent node(s)
		var el = this._sw;
		while(el = $(el.parentNode)) {
			if (Object.isFunction(el.getStyle) && el.getStyle('position') == 'absolute') {
				var parentOffset = $(this._sw.parentNode).cumulativeOffset();
  ...
  _openWindow: function(opt) {
    ...
    var opt = Object.extend({
      changePosition: true,
		...
		var sw = this._sw,
		    _options = this._options[this._sw.id],
		    position = this._getPositionAndSize();
		if (opt.isLoading) {
...
// go go go
document.observe('dom:loaded', function(){$sw._domLoaded = true});
